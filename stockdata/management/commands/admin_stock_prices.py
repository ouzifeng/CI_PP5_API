...
class Command(BaseCommand):
    help = (
        'Download EOD prices for listed exchanges and update Prices model if '
        'there is a change'
    )
            ...
            if response.status_code == 200:
                ...
                for item in data:
                    ...
                    if general:
                        ...
                        last_price = Prices.objects.filter(
                            general=general
                        ).first()

                        if last_price:
                            last_recorded_close = Decimal(
                                last_price.close
                            ).quantize(Decimal('.01'), rounding=ROUND_HALF_UP)

                            if last_recorded_close == current_close:
                                self.stdout.write(
                                    f"No change for {uid}, skipping update"
                                )
                                continue
                            else:
                                self.stdout.write(
                                    f"Price change detected for {uid}: "
                                    f"{last_recorded_close} to {current_close}"
                                )
                        else:
                            self.stdout.write(f"First price entry for {uid}")

                        # Proceed only if there is no last_price has changed
                        if not last_price or last_price.close != current_close:
                            # Define previous close if last_price exists
                            prev_close = (
                                last_price.close if last_price else None
                            )
                        ...
                        self.stdout.write(
                            f"No General entry found for {uid}, skipping"
                        )
            else:
                self.stdout.write(
                    f"Failed to download data for {exchange}, status code "
