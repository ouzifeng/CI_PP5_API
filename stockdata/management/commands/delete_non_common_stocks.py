...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if count == 0:
            self.stdout.write(
                self.style.SUCCESS('No non-"Common Stock" types to delete.')
            )
            return

        self.stdout.write(
            f'Found {count} stocks where type is not "Common Stock".'
        )
        ...
        for stock in stocks_to_delete.iterator():
            self.stdout.write(
                f'Deleting stock: {stock.name} ({stock.code}) with type: '
                f'{stock.type}'
            )
            stock.delete()
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'Successfully deleted {count} non-"Common Stock" stocks.'
            )
        )
