...
def import_tickers(api_token, exchange='LSE'):
    ...
    if response.status_code == 200:
        ...
        for entry in ticker_data:
            ...
            if len(primary_ticker) > 10:
                print(f"Skipping ticker {primary_ticker} because it's longer than 10 characters.")
                continue
    ...
    def handle(self, *args, **kwargs):
        ...
        for i, stock in enumerate(stocks, start=1):
            ...
            if response.status_code == 200:
                ...
                # Update or create General object
        ...
        for sheet_type in ['yearly', 'quarterly']:
            ...
            for key, sheet_data in income_statement_data.items():
                ...
                if created:
                    self.stdout.write(self.style.SUCCESS(f'Successfully imported data for {primary_ticker}.'))
                else:
                    self.stdout.write(self.style.SUCCESS(f'Successfully updated data for {primary_ticker}.'))
            else:
                self.stdout.write(self.style.ERROR(f'Failed to fetch data for stock: {stock.primary_ticker}. Status code: {response.status_code}'))
