...
class Command(BaseCommand):
    ...
    def filter_end_of_year_prices(self, stock_prices):
        ...
        for record in stock_prices:
            ...
            if start_year <= date.year <= current_year and date.month == 12:
                if date.year not in end_of_year_prices or date.day > end_of_year_prices[date.year]['date'].day:
    ...
    def handle(self, *args, **kwargs):
        for general in General.objects.all():
            ...
            sorted_prices = sorted(end_of_year_prices.items(), key=lambda x: x[0], reverse=True)
            adjusted_prices = self.adjust_for_splits(sorted_prices, splits_dividends)

            # Calculate CAGR
            cagr_5_years = self.calculate_cagr(adjusted_prices[4], adjusted_prices[0], 5) if len(adjusted_prices) >= 5 else None

            # Save the data to the database
            stock_prices_instance, created = StockPrices.objects.update_or_create(
                general=general,
                defaults={f'Y{idx + 1}': price for idx, price in enumerate(adjusted_prices)} | {
