...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if count == 0:
            self.stdout.write(
                self.style.SUCCESS(
                    'No stocks to delete based on dividend_yield criteria.'
                )
            )
            return

        self.stdout.write(
            f'Found {count} stocks with dividend_yield blank or 0.00.'
        )
        ...
        for stock in stocks_to_delete.iterator():
            self.stdout.write(
                f'Deleting stock: {stock.name} ({stock.code}) due to '
                f'dividend_share criteria.'
            )
            stock.delete()

        self.stdout.write(
            self.style.SUCCESS(
                f'Successfully deleted {count} stocks based on '
                f'dividend_share criteria.'
            )
        )
